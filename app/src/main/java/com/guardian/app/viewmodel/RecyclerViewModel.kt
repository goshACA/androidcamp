...
class RecyclerViewModel(application: Application) : ViewModel() {
    ...
    private val dataUseCase =
        GetRecyclerDataUseCase(
            RecyclerArticleRepository(
                RecyclerArticleDatabase.getInstance(
                    application
                )
            )
        )
    private val updateItemUseCase =
