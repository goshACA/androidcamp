...
import com.guardian.app.paging.DataSourceFactory
...
class RecyclerViewModel(
    application: Application, private val dataUseCase:
    GetRecyclerDataUseCase, private val updateItemUseCase:
    UpdateRecyclerItemUseCase
) : ViewModel() {
    val articleListLiveData: MutableLiveData<ArrayList<RecyclerArticleItem>> = MutableLiveData()


    val recyclerDatafactory = DataSourceFactory()
    val pagedListConfig: PagedList.Config = PagedList.Config.Builder()
        .setEnablePlaceholders(false)
        .setInitialLoadSizeHint(10)
        .setPageSize(10).build()
    val recyclerLiveData = LivePagedListBuilder(recyclerDatafactory, pagedListConfig)
        .build()
